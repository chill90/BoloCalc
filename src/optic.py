...
class Optic:
    ...
    def __init__(self, opt_chn, inp_dict, band_files=None):
        ...
        # Allowed band names
        self._band_names = ["REFL", "ABSO", "SPILL", "SCAT"]
    ...
    def _param_obj(self, param, min=None, max=None, inp_type=float):
        return pr.Parameter(
            self._log, param, self._inp_dict[param], min=min, max=max,
            band_ids=self._band_ids, inp_type=inp_type)
    ...
    def _store_param_dict(self):
        ...
        self._param_dict = {
            "elem": self._param_obj("Element", inp_type=str),
            "temp": self._param_obj("Temperature", min=0.0, max=np.inf),
            "abs": self._param_obj("Absorption", min=0.0, max=1.0),
            "refl": self._param_obj("Reflection", min=0.0, max=1.0),
            "thick": self._param_obj("Thickness", min=0.0, max=np.inf),
            "Index": self._param_obj("Index", min=0.0, max=np.inf),
            "ltan": self._param_obj("Loss Tangent", min=0.0, max=np.inf),
            "cond": self._param_obj("Conductivity", min=0.0, max=np.inf),
            "surfr": self._param_obj("Surface Rough", min=0.0, max=np.inf),
            "spill": self._param_obj("Spillover", min=0.0, max=1.0),
            "spillt": self._param_obj("Spillover Temp", min=0.0, max=np.inf),
            "scatf": self._param_obj("Scatter Frac", min=0.0, max=1.0),
            "scatt": self._param_obj("Scatter Temp", min=0.0, max=np.inf)}
        self._param_names = {
    ...
    def _store_tran(self):
        ...
        if ((self._param_dict["refl"].get_avg() == "BAND" and 
            "REFL" not in self._band_dict.keys()) or
            (self._param_dict["abs"].get_avg() == "BAND" and 
            "ABSO" not in self._band_dict.keys())):
            if "TRAN" in self._band_dict.keys():
                ...
                self._log.err(
                    "'BAND' defined for reflection and/or abosption for optic 
                    "'%s' but not enough absorption/reflection/transmission "
                    "band files defined to solve for all three quantities"
                    % (self._param_vals["elem"]))
        else:
        ...
        if self._param_dict["refl"].get_avg() == "BAND":
            if "REFL" in self._band_dict.keys():
                ...
                if self._refl is not None:
                    self._refl = np.array(
                        [r if r > 0 else 0. for r in self._refl])
                    self._refl = np.array(
                        [r if r < 1 else 1. for r in self._refl])
            else:
                self._log.err(
                    "'BAND' defined for reflection for optic '%s' but neither "
                    "reflection nor transmission band file found"
                    % (self._param_vals['elem'])
                )
