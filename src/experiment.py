...
class Experiment:
    def __init__(self, log, dir, nrealize=1, nobs=1, clcDet=1, specRes=1.e9, foregrounds=False):
        ...
        if self.fgnds:
            ...
            try:
                self.paramsDict = cl.OrderedDict({'Dust Temperature':       pr.Parameter(self.log, 'Dust Temperature',       dict['Dust Temperature'],       min=0.0,     max=np.inf),
                                                  'Dust Spec Index':        pr.Parameter(self.log, 'Dust Spec Index',        dict['Dust Spec Index'],        min=-np.inf, max=np.inf),
                                                  'Dust Amplitude':         pr.Parameter(self.log, 'Dust Amplitude',         dict['Dust Amplitude'],         min=0.0,     max=np.inf),
                                                  'Dust Scale Frequency':   pr.Parameter(self.log, 'Dust Scale Frequency',   dict['Dust Scale Frequency'],   min=0.0,     max=np.inf),
                                                  'Synchrotron Spec Index': pr.Parameter(self.log, 'Synchrotron Spec Index', dict['Synchrotron Spec Index'], min=-np.inf, max=np.inf),
                                                  'Synchrotron Amplitude':  pr.Parameter(self.log, 'Synchrotron Amplitude',  dict['Synchrotron Amplitude'],  min=0.0,     max=np.inf)})
            except KeyError:
                raise Exception("BoloCalc FATAL Exception: Failed to store parameters specified in '%s'" % (self.fgndFile))
        else:
    ...
    # ***** Public Methods *****
    def generate(self):
        #Generate foreground parameters
        ...
        for i in range(len(telescopeNames)):
            ...
            self.telescopes.update({telescopeNames[i]: tp.Telescope(self.log, telescopeDirs[i], fgndDict=self.params, nrealize=self.nrealize, nobs=self.nobs, clcDet=self.clcDet, specRes=self.specRes, foregrounds=self.fgnds)})
